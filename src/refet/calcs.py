...
def solar_time_rad(lon, time_mid, sc):
    ...
    Notes
    -----
    This function could be integrated into the solar_hour_angle() function since
    they are always called together (i.e. solar_hour_angle(solar_time_rad()).
    It was built independently of omega to eventually support having a separate
...
def ra_daily(lat, doy, method='asce'):
    ...
    omegas = sunset_hour_angle(lat, delta)
...
def ra_hourly(lat, lon, doy, time_mid, method='asce'):
    ...
    """
    omega = solar_hour_angle(solar_time_rad(lon, time_mid, seasonal_correction(doy)))
    delta = declination(doy, method)
    omegas = sunset_hour_angle(lat, delta)
...
def rso_hourly(ra, ea, pair, doy, time_mid, lat, lon, method='asce'):
    ...
    omega = solar_hour_angle(solar_time_rad(lon, time_mid, sc))
