...
def test_air_pressure_default(elev=s_args['elev'], pair=s_args['pair_asce']):
    assert float(calcs.air_pressure(elev)) == pytest.approx(pair)

def test_air_pressure_asce(elev=s_args['elev'], pair=s_args['pair_asce']):
    assert float(calcs.air_pressure(elev, method='asce')) == pytest.approx(pair)

def test_air_pressure_refet(elev=s_args['elev'], pair=s_args['pair']):
    assert float(calcs.air_pressure(elev, method='refet')) == pytest.approx(pair)
...
def test_sat_vapor_pressure(tdew, ea):
    assert float(calcs.sat_vapor_pressure(tdew)) == pytest.approx(ea)
...
def test_specific_humidity_daily(ea, pair, q):
    assert float(calcs.specific_humidity(ea, pair)) == pytest.approx(q)


def test_actual_vapor_pressure(q=d_args['q'], pair=s_args['pair'], ea=d_args['ea']):
    assert float(calcs.actual_vapor_pressure(q, pair)) == pytest.approx(ea)
...
def test_sha(solar_time=h_args['solar_time'], omega=h_args['omega']):
    assert float(calcs.sha(solar_time)) == pytest.approx(omega)
...
def test_sha_sunset(lat=s_args['lat'], delta=d_args['delta'], omega_s=d_args['omega_s']):
    assert float(calcs.sha_sunset(lat, delta)) == pytest.approx(omega_s)


def test_sha_sunset_high_lat():
    # Test that sha sunset is limited to [0, pi]
    # This occurs for angles > ~65 degrees in the summer and ~75 in the winter
    lat = 70 * (math.pi / 180)
    assert float(calcs.sha_sunset(lat, calcs.declination(182))) == pytest.approx(math.pi)
    assert float(calcs.sha_sunset(-lat, calcs.declination(182))) == pytest.approx(0)
    assert float(calcs.sha_sunset(lat, calcs.declination(1))) == pytest.approx(0)
...
def test_rso_daily_ra_zero(ea=d_args['ea'], pair=s_args['pair']):
    ...
    output = calcs.rso_daily(calcs.ra_daily(80 * math.pi / 180, 1), ea, pair, 1, 80 * math.pi / 180)
...
def test_fcd_daily(rs=d_args['rs'], rso=d_args['rso'], fcd=d_args['fcd']):
    assert float(calcs.fcd_daily(rs, rso)) == pytest.approx(fcd)


def test_fcd_daily_rso_zero():
    # Check that fcd returns 1 when Rso is zero
    assert float(calcs.fcd_daily(1, 0)) == pytest.approx(1.0)
    assert float(calcs.fcd_daily(0, 0)) == pytest.approx(1.0)
