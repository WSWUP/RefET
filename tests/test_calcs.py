...
def test_air_pressure_default(elev=s_args['elev'], pair=s_args['pair_asce']):
    assert float(calcs.air_pressure(elev)[0]) == pytest.approx(pair)

def test_air_pressure_asce(elev=s_args['elev'], pair=s_args['pair_asce']):
    assert float(calcs.air_pressure(elev, method='asce')[0]) == pytest.approx(pair)

def test_air_pressure_refet(elev=s_args['elev'], pair=s_args['pair']):
    assert float(calcs.air_pressure(elev, method='refet')[0]) == pytest.approx(pair)
...
def test_sat_vapor_pressure(tdew, ea):
    assert float(calcs.sat_vapor_pressure(tdew)[0]) == pytest.approx(ea)
...
def test_specific_humidity_daily(ea, pair, q):
    assert float(calcs.specific_humidity(ea, pair)[0]) == pytest.approx(q)


def test_actual_vapor_pressure(q=d_args['q'], pair=s_args['pair'], ea=d_args['ea']):
    assert float(calcs.actual_vapor_pressure(q, pair)[0]) == pytest.approx(ea)


def test_vpd(es=d_args['es'], ea=d_args['ea']):
    assert float(calcs.vpd(es, ea)) == pytest.approx(float(es-ea))
...
def test_es_slope_asce(tmin=d_args['tmin'], tmax=d_args['tmax'], es_slope=d_args['es_slope_asce']):
    output = calcs.es_slope(0.5 * (tmin + tmax), method='asce')
    assert float(output) == pytest.approx(es_slope)


def test_es_slope_refet(tmin=d_args['tmin'], tmax=d_args['tmax'], es_slope=d_args['es_slope']):
    output = calcs.es_slope(0.5 * (tmin + tmax), method='refet')
    assert float(output) == pytest.approx(es_slope)


def test_es_slope_hourly_asce(tmean=h_args['tmean'], es_slope=h_args['es_slope_asce']):
    assert float(calcs.es_slope(tmean, method='asce')) == pytest.approx(es_slope)


def test_es_slope_hourly_refet(tmean=h_args['tmean'], es_slope=h_args['es_slope']):
    assert float(calcs.es_slope(tmean, method='refet')) == pytest.approx(es_slope)


def test_precipitable_water(pair=s_args['pair'], ea=d_args['ea'], w=d_args['w']):
    assert float(calcs.precipitable_water(pair, ea)) == pytest.approx(w)


def test_doy_fraction(doy=d_args['doy'], expected=d_args['doy_frac']):
    assert float(calcs.doy_fraction(doy)) == pytest.approx(expected)


def test_declination_default(doy=d_args['doy'], delta=d_args['delta_asce']):
    assert float(calcs.declination(doy)) == pytest.approx(delta)


def test_declination_asce(doy=d_args['doy'], delta=d_args['delta_asce']):
    assert float(calcs.declination(doy, method='asce')) == pytest.approx(delta)


def test_declination_refet(doy=d_args['doy'], delta=d_args['delta']):
    assert float(calcs.declination(doy, method='refet')) == pytest.approx(delta)


def test_dr(doy=d_args['doy'], dr=d_args['dr']):
    assert float(calcs.dr(doy)) == pytest.approx(dr)


def test_seasonal_correction(doy=d_args['doy'], sc=d_args['sc']):
    assert float(calcs.seasonal_correction(doy)) == pytest.approx(sc)


def test_solar_time_rad(lon=s_args['lon'], time_mid=h_args['time'],
                        sc=d_args['sc'], expected=h_args['solar_time']):
    assert float(calcs.solar_time_rad(lon, time_mid, sc)) == pytest.approx(expected)


def test_solar_hour_angle(solar_time=h_args['solar_time'], omega=h_args['omega']):
    assert float(calcs.solar_hour_angle(solar_time)) == pytest.approx(omega)
...
def test_wrap(x, x_min, x_max, expected):
    value = calcs.wrap(x, x_min, x_max)
    assert pytest.approx(float(value)) == expected


def test_sunset_hour_angle(lat=s_args['lat'], delta=d_args['delta'], omega_s=d_args['omega_s']):
    assert float(calcs.sunset_hour_angle(lat, delta)) == pytest.approx(omega_s)


def test_sunset_hour_angle_high_lat():
    # Test that omega sunset is limited to [0, pi]
    # This occurs for angles > ~65 degrees in the summer and ~75 in the winter
    lat = 70 * (math.pi / 180)
    assert float(calcs.sunset_hour_angle(lat, calcs.declination(182))) == pytest.approx(math.pi)
    assert float(calcs.sunset_hour_angle(-lat, calcs.declination(182))) == pytest.approx(0)
    assert float(calcs.sunset_hour_angle(lat, calcs.declination(1))) == pytest.approx(0)


def test_ra_daily_default(lat=s_args['lat'], doy=d_args['doy'], ra=d_args['ra_asce']):
    assert float(calcs.ra_daily(lat, doy)) == pytest.approx(ra)


def test_ra_daily_asce(lat=s_args['lat'], doy=d_args['doy'], ra=d_args['ra_asce']):
    assert float(calcs.ra_daily(lat, doy, method='asce')) == pytest.approx(ra)


def test_ra_daily_refet(lat=s_args['lat'], doy=d_args['doy'], ra=d_args['ra']):
    assert float(calcs.ra_daily(lat, doy, method='refet')) == pytest.approx(ra)


def test_ra_daily_zero():
    # Ra can go to zero for winter DOY and/or high latitudes
    assert float(calcs.ra_daily(68 * (math.pi / 180), 1)) == 0
    assert float(calcs.ra_daily(80 * (math.pi / 180), 55)) == 0


def test_ra_hourly_default(lat=s_args['lat'], lon=s_args['lon'], doy=h_args['doy'],
                           time=h_args['time_mid'], ra=h_args['ra_asce']):
    assert float(calcs.ra_hourly(lat, lon, doy, time)) == pytest.approx(ra)

def test_ra_hourly_asce(lat=s_args['lat'], lon=s_args['lon'], doy=h_args['doy'],
                        time=h_args['time_mid'], ra=h_args['ra_asce']):
    output = calcs.ra_hourly(lat, lon, doy, time, method='asce')
    assert float(output) == pytest.approx(ra)

def test_ra_hourly_refet(lat=s_args['lat'], lon=s_args['lon'], doy=h_args['doy'],
                         time=h_args['time_mid'], ra=h_args['ra']):
    output = calcs.ra_hourly(lat, lon, doy, time, method='refet')
    assert float(output) == pytest.approx(ra)


def test_rso_daily(ra=d_args['ra'], ea=d_args['ea'], pair=s_args['pair'],
                   doy=d_args['doy'], lat=s_args['lat'], rso=d_args['rso']):
    assert float(calcs.rso_daily(ra, ea, pair, doy, lat)) == pytest.approx(rso)
...
def test_rso_daily_ra_zero(ea=d_args['ea'], pair=s_args['pair']):
    ...
    output = calcs.rso_daily(
        calcs.ra_daily(80 * math.pi / 180, 1), ea, pair, 1, 80 * math.pi / 180
    )
...
def test_rso_hourly_default(ra=h_args['ra'], ea=h_args['ea'], pair=s_args['pair'],
                            ...
                            rso=h_args['rso_asce']):
    output = calcs.rso_hourly(ra, ea, pair, doy, time, lat, lon)
    assert float(output) == pytest.approx(rso)
...
def test_rso_hourly_asce(ra=h_args['ra'], ea=h_args['ea'], pair=s_args['pair'],
                         ...
                         lat=s_args['lat'], lon=s_args['lon'],
                         rso=h_args['rso_asce']):
    output = calcs.rso_hourly(ra, ea, pair, doy, time, lat, lon,method='asce')
    assert float(output) == pytest.approx(rso)
...
def test_rso_hourly_refet(ra=h_args['ra'], ea=h_args['ea'], pair=s_args['pair'],
                          ...
                          lat=s_args['lat'], lon=s_args['lon'],
                          rso=h_args['rso']):
    output = calcs.rso_hourly(ra, ea, pair, doy, time, lat, lon, method='refet')
    assert float(output) == pytest.approx(rso)
...
def test_rso_simple(ra, elev, rso):
    assert float(calcs.rso_simple(ra, elev)) == pytest.approx(rso)


def test_fcd_daily(rs=d_args['rs'], rso=d_args['rso'], fcd=d_args['fcd']):
    assert float(calcs.fcd_daily(rs, rso)[0]) == pytest.approx(fcd)


def test_fcd_daily_rso_zero():
    # Check that fcd returns 1 when Rso is zero
    assert float(calcs.fcd_daily(1, 0)[0]) == pytest.approx(1.0)
    assert float(calcs.fcd_daily(0, 0)[0]) == pytest.approx(1.0)
...
def test_fcd_hourly_default(rs=h_args['rs'], rso=h_args['rso'],
                            ...
                            lat=s_args['lat'], lon=s_args['lon'],
                            fcd=h_args['fcd_asce']):
    output = calcs.fcd_hourly(rs, rso, doy, time, lat, lon)
    assert float(output[0]) == pytest.approx(fcd)
...
def test_fcd_hourly_asce(rs=h_args['rs'], rso=h_args['rso'],
                         ...
                         lat=s_args['lat'], lon=s_args['lon'],
                         fcd=h_args['fcd_asce']):
    output = calcs.fcd_hourly(rs, rso, doy, time, lat, lon, method='asce')
    assert float(output[0]) == pytest.approx(fcd)
...
def test_fcd_hourly_refet(rs=h_args['rs'], rso=h_args['rso'],
                          ...
                          lat=s_args['lat'], lon=s_args['lon'],
                          fcd=h_args['fcd']):
    output = calcs.fcd_hourly(rs, rso, doy, time, lat, lon, method='refet')
    assert float(output[0]) == pytest.approx(fcd)


def test_fcd_hourly_night(rs=h_args['rs'], rso=h_args['rso'], doy=h_args['doy'], time=6,
...
def test_rnl_daily(tmin=d_args['tmin'], tmax=d_args['tmax'], ea=d_args['ea'],
    ...
    assert float(calcs.rnl_daily(tmax, tmin, ea, fcd)) == pytest.approx(rnl)


def test_rnl_hourly(tmean=h_args['tmean'], ea=h_args['ea'], fcd=h_args['fcd'], rnl=h_args['rnl']):
    assert float(calcs.rnl_hourly(tmean, ea, fcd)) == pytest.approx(rnl)


def test_rn_daily(rs=d_args['rs'], rnl=d_args['rnl'], rn=d_args['rn']):
    assert float(calcs.rn_daily(rs, rnl)) == pytest.approx(rn)


def test_rn_hourly(rs=h_args['rs'], rnl=h_args['rnl'], rn=h_args['rn']):
    assert float(calcs.rn_hourly(rs, rnl)) == pytest.approx(rn)
...
def test_wind_height_adjust(uz, zw, u2):
    assert float(calcs.wind_height_adjust(uz, zw)) == pytest.approx(u2)


def test_wind_height_adjust_2m(uz=2.5, zw=2.0, u2=2.5):
    assert float(calcs.wind_height_adjust(uz, zw)) == pytest.approx(u2, abs=0.001)
...
def test_etsz(rn, g, tmean, u2, es, ea, es_slope, pair, cn, cd, etsz):
    output = calcs.etsz(rn, g, tmean, u2, es - ea, es_slope, 0.000665 * pair, cn, cd)
